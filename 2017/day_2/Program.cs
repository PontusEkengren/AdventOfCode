...
2257	155	734	228	204	2180	175	2277	180	2275	2239	2331	2278	1763	112	2054";
        ...
        {
            Console.WriteLine("Puzzle 2 50%");

            string[] rows = puzzleInput.Split(new[] { Environment.NewLine }, StringSplitOptions.None);
            int checksum = 0;
            foreach (string row in rows)
            {
                string[] charInRow = row.Split(' ', '\t');
                int[] rowValues = Array.ConvertAll(charInRow, s =>  int.Parse(s));
        ...
        {
            Console.WriteLine("Puzzle 2 100%");
            string[] rows = puzzleInput.Split(new[] { Environment.NewLine }, StringSplitOptions.None);
            int checksum = 0;
            foreach (string row in rows)
            {
                ...
                checksum += divideValidValue(rowValues);
            }
            Console.WriteLine("checksum: " + checksum);
        ...
        private static int divideValidValue(int[] rowValues)
        {
            ...
            for (int i = 0; i < rowValues.Length; i++)
            {
                ...
                {
                    if (i != j && rowValues[i] % rowValues[j] == 0)
                    {
                        return rowValues[i] / rowValues[j];
                    }
